<template>
    <div>
      <button
        ...
        :class="['flex-row items-center space-x-3 text-black w-full bg-white py-4 p-2 hover:bg-gray-100 active:bg-gray-200', { 'collapsed': !isOpen }]"
        :aria-expanded="isOpen"
        :aria-controls="collapseId"
      >
        <slot name="title" />
        <svg
          class="w-3 mt-2 transition-all duration-200 transform"
          :class="{
            'rotate-180': isOpen,
            'rotate-0': !isOpen,
          }"
      ...
      <div class="flex-row justify-center items-center" v-show="isOpen" :id="collapseId">
        <slot name="content" />
      </div>
    </div>
  </template>
