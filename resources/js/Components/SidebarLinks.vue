<template>
    <SidebarAccordion :uniqueKey="'accordion1'">
    <template #title>
        <svg width="37px" height="37px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="24" height="24" />
        <path fill-rule="evenodd" clip-rule="evenodd" d="M5 9.5C5 7.01472 7.01472 5 9.5 5C11.9853 5 14 7.01472 14 9.5C14 11.9853 11.9853 14 9.5 14C7.01472 14 5 11.9853 5 9.5Z" fill="#323232"/>
        <path d="M14.3675 12.0632C14.322 12.1494 14.3413 12.2569 14.4196 12.3149C15.0012 12.7454 15.7209 13 16.5 13C18.433 13 20 11.433 20 9.5C20 7.567 18.433 6 16.5 6C15.7209 6 15.0012 6.2546 14.4196 6.68513C14.3413 6.74313 14.322 6.85058 14.3675 6.93679C14.7714 7.70219 15 8.5744 15 9.5C15 10.4256 14.7714 11.2978 14.3675 12.0632Z" fill="#323232"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M4.64115 15.6993C5.87351 15.1644 7.49045 15 9.49995 15C11.5112 15 13.1293 15.1647 14.3621 15.7008C15.705 16.2847 16.5212 17.2793 16.949 18.6836C17.1495 19.3418 16.6551 20 15.9738 20H3.02801C2.34589 20 1.85045 19.3408 2.05157 18.6814C2.47994 17.2769 3.29738 16.2826 4.64115 15.6993Z" fill="#323232"/>
        <path d="M14.8185 14.0364C14.4045 14.0621 14.3802 14.6183 14.7606 14.7837V14.7837C15.803 15.237 16.5879 15.9043 17.1508 16.756C17.6127 17.4549 18.33 18 19.1677 18H20.9483C21.6555 18 22.1715 17.2973 21.9227 16.6108C21.9084 16.5713 21.8935 16.5321 21.8781 16.4932C21.5357 15.6286 20.9488 14.9921 20.0798 14.5864C19.2639 14.2055 18.2425 14.0483 17.0392 14.0008L17.0194 14H16.9997C16.2909 14 15.5506 13.9909 14.8185 14.0364Z" fill="#323232"/>
        </svg>
    </template>
    <template #content>
        <Tooltip>
            <template #button>
                <SidebarLink link="employees.index">
                    <svg fill="#fff" width="37px" height="37px" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                        <path d="M73.8,29.7H26.3a6,6,0,0,0-6,6V64.3a6,6,0,0,0,6,6H73.7a6,6,0,0,0,6-6V35.7A6.13,6.13,0,0,0,73.8,29.7ZM38.9,35.1a7.6,7.6,0,1,1-7.6,7.6A7.66,7.66,0,0,1,38.9,35.1Zm9.8,28.3H29.2c-1.7,0-2-1.1-2-1.6a11.45,11.45,0,0,1,11.4-9.9H39a11.45,11.45,0,0,1,11.4,9.9C50.7,62.3,50.4,63.4,48.7,63.4Zm23.1-6.9a2,2,0,0,1-2,2H61a2,2,0,0,1-2-2v-3a2,2,0,0,1,2-2h8.8a2,2,0,0,1,2,2Zm0-11a2,2,0,0,1-2,2H55a2,2,0,0,1-2-2v-3a2,2,0,0,1,2-2H69.8a2,2,0,0,1,2,2Z"/>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Employés
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="teams.index">
                    <svg fill="#fff" height="37px" width="37px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="0 0 512 512" xml:space="preserve">
                    <g>
                        <g>
                            <g>
                                <path d="M96.711,245.262c37.127,0,67.333-30.206,67.333-67.333c0-37.127-30.206-67.333-67.333-67.333
                                    s-67.333,30.205-67.333,67.333C29.379,215.056,59.584,245.262,96.711,245.262z"/>
                                <path d="M256,245.262c37.127,0,67.333-30.206,67.333-67.333c0-37.127-30.206-67.333-67.333-67.333
                                    c-37.127,0-67.333,30.205-67.333,67.333C188.667,215.056,218.873,245.262,256,245.262z"/>
                                <path d="M415.289,245.262c37.127,0,67.333-30.206,67.333-67.333c0-37.127-30.206-67.333-67.333-67.333
                                    s-67.333,30.205-67.333,67.333C347.956,215.056,378.162,245.262,415.289,245.262z"/>
                                <path d="M335.644,298.668c-38.588-55.933-120.761-55.844-159.289,0C122.534,220.656,0,258.922,0,353.444v30.893
                                    c0,9.425,7.641,17.067,17.067,17.067c8.71,0,468.095,0,477.867,0c9.425,0,17.067-7.641,17.067-17.067v-30.893
                                    C512,258.922,389.452,220.675,335.644,298.668z"/>
                            </g>
                        </g>
                    </g>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Teams
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="employees.show_absences">
                    <svg fill="#fff" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                    width="37px" height="37px" viewBox="0 0 261.258 261.259"
                    xml:space="preserve">
                    <g>
                        <g>
                            <path d="M50.591,141.681c-0.348-0.485-0.917-0.761-1.559-0.761h-5.131c-1.057,0-2.075,0.761-2.371,1.778l-5.691,19.723
                                l-5.64-19.714c-0.292-1.017-1.305-1.787-2.364-1.787H22.5c-0.639,0-1.208,0.28-1.559,0.761c-0.348,0.485-0.425,1.115-0.219,1.718
                                l10.361,30.644c0.336,0.976,1.363,1.708,2.395,1.708h4.492c1.034,0,2.065-0.732,2.394-1.708l10.447-30.64
                                C51.016,142.801,50.941,142.171,50.591,141.681z"/>
                            <path d="M69.101,142.614c-0.348-0.98-1.358-1.689-2.399-1.689h-5.759c-1.045,0-2.056,0.709-2.403,1.689l-10.94,30.648
                                c-0.214,0.611-0.142,1.246,0.201,1.727c0.345,0.49,0.912,0.771,1.559,0.771h5.181c1.036,0,2.065-0.742,2.389-1.727l1.72-4.942
                                l10.15,0.079l1.559,4.854c0.315,0.994,1.342,1.736,2.39,1.736h5.437c0.646,0,1.213-0.28,1.563-0.766
                                c0.343-0.494,0.416-1.115,0.201-1.722L69.101,142.614z M66.209,161.109h-5.022l2.546-7.715L66.209,161.109z"/>
                            <path d="M96.599,148.485c2.023,0,3.484,0.551,4.459,1.689c0.438,0.514,0.826,1.26,1.157,2.23c0.343,0.989,1.342,1.713,2.369,1.713
                                h4.87c0.618,0,1.195-0.266,1.587-0.714c0.378-0.443,0.546-1.026,0.459-1.605c-0.282-1.886-1.062-3.771-2.327-5.609
                                c-2.726-3.902-7.04-5.881-12.832-5.881c-4.354,0-7.997,1.47-10.83,4.359c-3.215,3.29-4.852,7.98-4.852,13.936
                                c0,5.517,1.436,9.95,4.264,13.161c2.868,3.258,6.83,4.9,11.784,4.9c4.025,0,7.391-1.228,9.995-3.645
                                c2.345-2.18,3.904-4.976,4.63-8.312c0.13-0.598-0.004-1.199-0.378-1.657c-0.376-0.471-0.945-0.732-1.566-0.732h-4.828
                                c-1.036,0-2.03,0.732-2.361,1.741c-0.348,1.078-0.754,1.908-1.214,2.487c-0.989,1.265-2.422,1.876-4.377,1.876
                                c-1.907,0-3.44-0.747-4.684-2.286c-1.286-1.578-1.939-4.08-1.939-7.421c0-3.375,0.63-5.983,1.874-7.757
                                C93.029,149.293,94.578,148.485,96.599,148.485z"/>
                            <path d="M134.22,142.614c-0.348-0.98-1.358-1.689-2.399-1.689h-5.764c-1.027,0-2.061,0.728-2.399,1.689l-10.944,30.648
                                c-0.215,0.611-0.142,1.246,0.201,1.727c0.345,0.49,0.913,0.771,1.559,0.771h5.18c1.036,0,2.065-0.742,2.39-1.727l1.72-4.942
                                l10.151,0.079l1.559,4.854c0.315,0.994,1.341,1.736,2.389,1.736h5.444c0.644,0,1.204-0.28,1.559-0.761
                                c0.341-0.49,0.42-1.115,0.206-1.727L134.22,142.614z M126.312,161.109l2.546-7.715l2.476,7.715H126.312z"/>
                            <path d="M173.634,140.92h-4.406c-1.186,0-2.151,0.966-2.151,2.151v17.156l-10.175-17.875c-0.48-0.831-1.507-1.433-2.469-1.433
                                h-5.189c-1.186,0-2.156,0.966-2.156,2.151v30.537c0,1.186,0.971,2.151,2.156,2.151h4.406c1.185,0,2.146-0.966,2.146-2.151v-17.591
                                l10.506,18.314c0.48,0.83,1.517,1.428,2.469,1.428h4.872c1.186,0,2.151-0.966,2.151-2.151v-30.537
                                C175.781,141.886,174.819,140.92,173.634,140.92z"/>
                            <path d="M194.281,148.485c2.025,0,3.486,0.551,4.462,1.689c0.438,0.514,0.826,1.26,1.157,2.23
                                c0.346,0.989,1.345,1.713,2.371,1.713h4.868c0.611,0,1.189-0.266,1.582-0.714c0.383-0.438,0.551-1.018,0.467-1.605
                                c-0.285-1.881-1.064-3.762-2.329-5.609c-2.726-3.902-7.043-5.881-12.83-5.881c-4.359,0-7.999,1.47-10.832,4.359
                                c-3.216,3.29-4.85,7.98-4.85,13.936c0,5.517,1.434,9.95,4.262,13.161c2.87,3.258,6.828,4.9,11.784,4.9
                                c4.022,0,7.393-1.228,9.997-3.645c2.343-2.18,3.901-4.976,4.63-8.312c0.13-0.598-0.01-1.199-0.378-1.657
                                c-0.379-0.471-0.948-0.732-1.568-0.732h-4.831c-1.036,0-2.025,0.732-2.356,1.741c-0.35,1.078-0.756,1.908-1.218,2.487
                                c-0.99,1.26-2.418,1.876-4.378,1.876c-1.904,0-3.436-0.747-4.682-2.286c-1.283-1.578-1.937-4.08-1.937-7.421
                                c0-3.375,0.626-5.983,1.872-7.757C190.715,149.293,192.265,148.485,194.281,148.485z"/>
                            <path d="M230.544,142.483l-4.994,10.939l-5.218-10.958c-0.43-0.9-1.456-1.545-2.45-1.545h-6.081c-0.687,0-1.251,0.299-1.568,0.831
                                c-0.317,0.527-0.299,1.176,0.028,1.778l10.305,18.611c0.158,0.29,0.312,0.892,0.312,1.214v10.259c0,1.185,0.967,2.151,2.151,2.151
                                h4.873c1.186,0,2.146-0.967,2.146-2.151v-10.259c0-0.326,0.154-0.934,0.312-1.232l9.95-18.598c0.322-0.603,0.322-1.256,0.01-1.778
                                s-0.892-0.826-1.568-0.826h-5.773C231.982,140.92,230.959,141.578,230.544,142.483z"/>
                            <path d="M28.803,100.125h-7.297C9.649,100.125,0,109.774,0,121.631v73.702c0,11.854,9.649,21.507,21.506,21.507h218.247
                                c11.858,0,21.506-9.652,21.506-21.507v-73.702c0-11.856-9.647-21.506-21.506-21.506h-7.295V44.419h-14.337l0,0v55.706H43.14
                                V44.419l0,0H28.803V100.125z M246.917,121.631v73.702c0,3.953-3.216,7.169-7.169,7.169H21.506c-3.953,0-7.169-3.216-7.169-7.169
                                v-73.702c0-3.953,3.216-7.168,7.169-7.168h7.297h203.646h7.299C243.706,114.462,246.917,117.678,246.917,121.631z"/>
                        </g>
                    </g>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Absences
            </template>
        </Tooltip>
    </template>
</SidebarAccordion>
<SidebarAccordion :uniqueKey="'accordion2'">
    <template #title>
        <svg width="37px" height="37px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M5 6V15.8C5 16.9201 5 17.4802 5.21799 17.908C5.40973 18.2843 5.71569 18.5903 6.09202 18.782C6.51984 19 7.07989 19 8.2 19H15.8C16.9201 19 17.4802 19 17.908 18.782C18.2843 18.5903 18.5903 18.2843 18.782 17.908C19 17.4802 19 16.9201 19 15.8V6M5 6C5 6 5 3 12 3C19 3 19 6 19 6M5 6H19M5 13H19M17 21V19M7 21V19M8 16H8.01M16 16H16.01" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
    </template>
    <template #content>
        <Tooltip>
            <template #button>
                <SidebarLink link="vehicles.index">
                    <svg width="37px" height="37px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M5 6V15.8C5 16.9201 5 17.4802 5.21799 17.908C5.40973 18.2843 5.71569 18.5903 6.09202 18.782C6.51984 19 7.07989 19 8.2 19H15.8C16.9201 19 17.4802 19 17.908 18.782C18.2843 18.5903 18.5903 18.2843 18.782 17.908C19 17.4802 19 16.9201 19 15.8V6M5 6C5 6 5 3 12 3C19 3 19 6 19 6M5 6H19M5 13H19M17 21V19M7 21V19M8 16H8.01M16 16H16.01" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Vehicles
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="drivers.index">
                    <svg fill="#fff" height="37px" width="37px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="0 0 299.044 299.044" xml:space="preserve">
                    <g id="XMLID_1569_">
                        <g>
                            <g>
                                <path d="M291.037,52.054h-19.889V16.965c0-3.706-3.004-6.709-6.709-6.709H136.054c-3.706,0-6.709,3.004-6.709,6.709v35.088
                                    h-11.471c10.466,7.699,16.76,19.859,16.825,32.915l0.414,82.514c0.682,0.096,0.59,0.073,4.298,0.073v13.954
                                    c0,8.795,7.13,15.925,15.925,15.925c8.795,0,15.925-7.13,15.925-15.925v-13.954h57.968v13.954c0,8.795,7.13,15.925,15.925,15.925
                                    s15.925-7.13,15.925-15.925v-13.954h3.359c3.706,0,6.709-3.004,6.709-6.709v-47.03h19.889c3.706,0,6.709-3.004,6.709-6.709
                                    V58.763h0C297.746,55.058,294.743,52.054,291.037,52.054z M146.671,47.061c0-2.215,1.796-4.011,4.011-4.011h99.129
                                    c2.215,0,4.011,1.796,4.011,4.011v45.523c0,2.215-1.796,4.011-4.011,4.011h-99.129c-2.215,0-4.011-1.796-4.011-4.011V47.061z
                                    M157.346,155.366c-8.111,0-14.686-6.575-14.686-14.687c0-8.111,6.575-14.687,14.686-14.687c8.111,0,14.687,6.575,14.687,14.687
                                    C172.033,148.79,165.458,155.366,157.346,155.366z M243.146,155.366c-8.111,0-14.686-6.575-14.686-14.687
                                    c0-8.111,6.575-14.687,14.686-14.687c8.111,0,14.687,6.575,14.687,14.687C257.833,148.79,251.258,155.366,243.146,155.366z"/>
                                <path d="M61.818,49.226c13.554,0,24.613-11.003,24.613-24.613C86.431,11.019,75.411,0,61.818,0S37.205,11.019,37.205,24.613
                                    C37.205,38.257,48.297,49.226,61.818,49.226z"/>
                                <path d="M98.088,57.437c-3.156-0.515,1.046-0.365-68.249-0.365c-15.419,0-28.026,12.544-28.104,27.963l-0.438,87.247
                                    c-0.033,6.559,5.257,11.903,11.816,11.935c0.02,0,0.041,0,0.061,0c6.531,0,11.842-5.278,11.875-11.816l0.438-87.248
                                    c0-0.001,0-0.001,0-0.001c0.006-1.264,1.035-2.284,2.299-2.281c1.264,0.003,2.287,1.03,2.287,2.294
                                    c0.001,28.378,0.005,186.637,0.005,199.627c0,7.871,6.38,14.251,14.251,14.251s14.251-6.38,14.251-14.251V170.882h6.154v113.911
                                    c0,7.871,6.38,14.251,14.251,14.251s14.251-6.38,14.251-14.251c0-195.493-0.253-74.027-0.259-199.625
                                    c0-1.37,1.108-2.481,2.478-2.486c1.37-0.005,2.485,1.101,2.492,2.472l0.438,87.248c0.033,6.539,5.344,11.816,11.875,11.816
                                    c0.019,0,0.041,0,0.061,0c6.559-0.033,11.849-5.377,11.816-11.935l-0.438-87.248C121.638,71.352,111.601,59.643,98.088,57.437z"
                                    />
                            </g>
                        </g>
                    </g>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Chauffeurs
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="terminals.index">
                    <svg fill="#fff" height="37px" width="37px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="0 0 512 512" xml:space="preserve">
                    <g>
                        <g>
                            <g>
                                <path d="M474.194,199.093h-48.167c-4.422,0-8.005,3.583-8.005,8.005s3.583,8.005,8.005,8.005h48.167
                                    c4.422,0,8.005-3.583,8.005-8.005S478.615,199.093,474.194,199.093z"/>
                                <path d="M450.12,192.344c16.767,0,30.409-13.641,30.409-30.41s-13.641-30.41-30.409-30.41c-16.768,0-30.411,13.641-30.411,30.41
                                    S433.352,192.344,450.12,192.344z M450.12,147.533c7.94,0,14.399,6.46,14.399,14.4c0,7.94-6.459,14.4-14.399,14.4
                                    c-7.941,0-14.401-6.46-14.401-14.4C435.719,153.993,442.179,147.533,450.12,147.533z"/>
                                <path d="M257.91,283.049c-3.235,0-5.857,2.625-5.857,5.859c0,3.237,2.622,5.859,5.857,5.859c3.238,0,5.861-2.622,5.861-5.859
                                    C263.772,285.674,261.149,283.049,257.91,283.049z"/>
                                <circle cx="130.812" cy="288.909" r="5.859"/>
                                <circle cx="300.279" cy="288.909" r="5.859"/>
                                <path d="M499.721,396.846h-88.53V245.63h85.847c4.422,0,8.005-3.583,8.005-8.005V117.836c0-4.421-3.583-8.005-8.005-8.005
                                    h-73.175c0.162-1.001,0.27-2.019,0.27-3.064V94.369c0-10.517-8.557-19.074-19.074-19.074H26.031
                                    c-10.517,0-19.073,8.557-19.073,19.074v12.399c0,8.372,5.428,15.484,12.943,18.044v272.035h-7.623
                                    C5.508,396.846,0,402.354,0,409.126v15.299c0,6.772,5.508,12.28,12.279,12.28h487.441c6.771,0,12.279-5.508,12.279-12.28v-15.299
                                    C512,402.354,506.492,396.846,499.721,396.846z M411.19,125.841h77.842v103.778H411.19V125.841z M22.968,94.369
                                    c0-1.662,1.402-3.064,3.063-3.064h379.027c1.662,0,3.064,1.402,3.064,3.064v12.399c0,1.662-1.402,3.064-3.064,3.064H26.031
                                    c-1.661,0-3.063-1.402-3.063-3.064V94.369z M395.181,125.841v271.005h-7.97V125.841H395.181z M85.365,347.242
                                    c-5.776,0-10.474-4.699-10.474-10.477c0-5.776,4.698-10.474,10.474-10.474h260.359c5.776,0,10.474,4.698,10.474,10.474
                                    c0,5.777-4.698,10.477-10.474,10.477H85.365z M323.229,363.251v33.595h-4.723v-33.595H323.229z M302.496,363.251v33.595H128.594
                                    v-33.595H302.496z M112.584,363.251v33.595h-4.723v-33.595H112.584z M91.853,363.253v33.594H59.889v-52.859
                                    c3.152,11.103,13.375,19.265,25.475,19.265H91.853z M93.37,310.28v-25.238c0-5.529,4.498-10.027,10.025-10.027h224.301
                                    c5.528,0,10.025,4.499,10.025,10.027v25.238H93.37z M339.238,363.251h6.487c12.101,0,22.326-8.162,25.476-19.269v52.863h-31.964
                                    V363.251z M353.73,311.519v-26.478c0-14.356-11.678-26.037-26.035-26.037H103.395c-14.355,0-26.035,11.679-26.035,26.037v26.478
                                    c-8.434,2.681-15.046,9.48-17.471,18.025V245.63h311.312v83.918C368.777,321.001,362.165,314.2,353.73,311.519z M59.889,229.62
                                    V125.842h135.661V229.62H59.889z M211.56,229.62V125.842h7.97V229.62H211.56z M235.538,229.62V125.842h135.662V229.62H235.538z
                                    M43.88,125.841v271.005h-7.97V125.841H43.88z M495.991,420.696H16.01v-7.84h479.981V420.696z"/>
                                <circle cx="173.177" cy="288.909" r="5.859"/>
                                <circle cx="215.545" cy="288.909" r="5.859"/>
                            </g>
                        </g>
                    </g>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Paradas
            </template>
        </Tooltip>
    </template>
</SidebarAccordion>
<SidebarAccordion :uniqueKey="'accordion3'">
    <template #title>
        <svg fill="#000000" width="37px" height="37px" viewBox="0 0 1920 1920" xmlns="http://www.w3.org/2000/svg">
            <path d="m773.596 1069.654 711.086 711.085c39.632 39.632 104.336 39.632 144.078 0l138.276-138.385c19.268-19.269 29.888-44.778 29.888-71.93 0-27.26-10.62-52.77-29.888-72.039l-698.714-698.714 11.495-32.625c63.5-178.675 18.284-380.45-115.284-514.018-123.715-123.605-305.126-171.01-471.648-128.313l272.281 272.282c32.516 32.406 50.362 75.652 50.362 121.744 0 45.982-17.846 89.227-50.362 121.744L654.48 751.17c-67.222 67.003-176.375 67.003-243.488 0L138.711 478.889c-42.589 166.522 4.707 347.934 128.313 471.648 123.714 123.715 306.22 172.325 476.027 127.218l30.545-8.101ZM1556.611 1920c-54.084 0-108.168-20.692-149.333-61.857L740.095 1190.96c-198.162 41.712-406.725-19.269-550.475-163.019C14.449 852.771-35.256 582.788 65.796 356.27l32.406-72.696 390.194 390.193c24.414 24.305 64.266 24.305 88.68 0l110.687-110.686c11.824-11.934 18.283-27.59 18.283-44.34 0-16.751-6.46-32.516-18.283-44.34L297.569 84.207 370.265 51.8C596.893-49.252 866.875.453 1041.937 175.515c155.026 155.136 212.833 385.157 151.851 594.815l650.651 650.651c39.961 39.852 61.967 92.95 61.967 149.443 0 56.383-22.006 109.482-61.967 149.334l-138.275 138.385c-41.275 41.165-95.36 61.857-149.553 61.857Z" fill-rule="evenodd"/>
        </svg>
    </template>
    <template #content>
        <Tooltip>
            <template #button>
                <SidebarLink link="users.index" v-if="role == 'administrator'">
                    <svg width="37px" height="37px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="12" cy="6" r="4" stroke="#fff" stroke-width="1.5"/>
                    <path d="M15 20.6151C14.0907 20.8619 13.0736 21 12 21C8.13401 21 5 19.2091 5 17C5 14.7909 8.13401 13 12 13C15.866 13 19 14.7909 19 17C19 17.3453 18.9234 17.6804 18.7795 18" stroke="#fff" stroke-width="1.5" stroke-linecap="round"/>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Utilisateurs
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="projects.index">
                    <svg width="37px" height="37px" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <title>project-new</title>
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="Combined-Shape" fill="#fff" transform="translate(64.000000, 34.346667)">
                                <path d="M192,-7.10542736e-15 L384,110.851252 L384,242.986 L341.333,242.986 L341.333,157.655 L213.333,231.555 L213.333,431.088 L192,443.405007 L0,332.553755 L0,110.851252 L192,-7.10542736e-15 Z M341.333333,264.32 L341.333,328.32 L405.333333,328.32 L405.333333,370.986667 L341.333,370.986 L341.333333,434.986667 L298.666667,434.986667 L298.666,370.986 L234.666667,370.986667 L234.666667,328.32 L298.666,328.32 L298.666667,264.32 L341.333333,264.32 Z M42.666,157.654 L42.6666667,307.920144 L170.666,381.82 L170.666,231.555 L42.666,157.654 Z M192,49.267223 L66.1333333,121.936377 L192,194.605531 L317.866667,121.936377 L192,49.267223 Z">
        
                    </path>
                            </g>
                        </g>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des projets
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="positions.index">
                    <svg fill="#fff" width="37px" height="37px" viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg" id="memory-menu-up-fill"><path d="M5 13V14H17V13H16V12H15V11H14V10H13V9H12V8H10V9H9V10H8V11H7V12H6V13" /></svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Positions
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="stations.index">
                    <svg version="1.1" fill="#fff" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="0 0 512 512"  xml:space="preserve">
                    <g>
                        <path class="st0" d="M92.247,210.052v-60.734c8.987-2.492,15.615-10.647,15.615-20.43c0-3.633-0.995-6.997-2.6-9.991l57.578-49.122
                            l-5.964-17.297l-65.37,55.764c-1.59-0.378-3.225-0.632-4.93-0.632c-11.75,0-21.279,9.536-21.279,21.278
                            c0,7.576,3.974,14.181,9.922,17.954v63.21H49.373v32.735h413.254v-32.735H92.247z M95.795,128.889c0,2.924-1.436,5.416-3.549,7.098
                            l0,0c-0.772,0.617-1.62,1.126-2.538,1.496c-0.988,0.362-2.022,0.633-3.132,0.633c-5.093,0-9.22-4.12-9.22-9.228
                            c0-0.995,0.277-1.92,0.578-2.824h-0.007c0.501-1.528,1.311-2.862,2.469-3.919l0,0c0.81-0.74,1.75-1.358,2.785-1.79
                            c1.057-0.417,2.191-0.679,3.395-0.679c1.18,0,2.291,0.262,3.325,0.671c0.648,0.254,1.203,0.656,1.758,1.057
                            c0.286,0.193,0.594,0.355,0.85,0.57c0.725,0.656,1.365,1.412,1.851,2.268c0.008,0.016,0.008,0.016,0.016,0.031
                            c0.516,0.903,0.887,1.913,1.088,2.978l0,0C95.572,127.801,95.795,128.31,95.795,128.889z"/>
                        <polygon class="st0" points="409.524,305.026 409.524,512 450.592,512 450.592,254.831 409.524,254.831 	"/>
                        <polygon class="st0" points="61.416,512 102.492,512 102.492,305.026 102.492,254.831 61.416,254.831 	"/>
                        <rect x="114.528" y="254.831" class="st0" width="283.863" height="50.195"/>
                        <polygon class="st0" points="172.793,58.018 191.656,112.811 269.34,35.127 214.562,16.256 209.254,21.556 187.698,0 
                            156.544,31.161 178.108,52.71 	"/>
                    </g>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Stations
            </template>
        </Tooltip>
        <Tooltip>
            <template #button>
                <SidebarLink link="departments.index">
                    <svg width="37px" height="37px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M3 21H21" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M19 21V15V7C19 5.11438 19 4.17157 18.4142 3.58579C17.8284 3 16.8856 3 15 3H12H9C7.11438 3 6.17157 3 5.58579 3.58579C5 4.17157 5 5.11438 5 7V15V21" stroke="#fff" stroke-width="2" stroke-linejoin="round"/>
                    <path d="M9 12L10 12" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M9 16L10 16" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M14 8L15 8" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M14 12L15 12" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M14 16L15 16" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </SidebarLink>
            </template>
            <template #title>
                Gestion Des Departemets
            </template>
        </Tooltip>
    </template>
</SidebarAccordion>
<Tooltip>
    <template #button>
        <SidebarLink link="help.index">
            <svg width="37px" height="37px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="12" cy="12" r="10" stroke="#fff" stroke-width="1.5"/>
            <path d="M10.125 8.875C10.125 7.83947 10.9645 7 12 7C13.0355 7 13.875 7.83947 13.875 8.875C13.875 9.56245 13.505 10.1635 12.9534 10.4899C12.478 10.7711 12 11.1977 12 11.75V13" stroke="#fff" stroke-width="1.5" stroke-linecap="round"/>
            <circle cx="12" cy="16" r="1" fill="#fff"/>
            </svg>
        </SidebarLink>
    </template>
    <template #title>
        Aide
    </template>
</Tooltip>
</template>
<script>
    import SidebarAccordion from '@/Components/SidebarAccordion.vue'
    import SidebarLink from '@/Components/SidebarLink.vue';
    import Tooltip from '@/Components/Tooltip.vue'
    export default {
        props: {
            role: String,
        },
        components: {
            Tooltip, SidebarAccordion, SidebarLink
        },
    };
</script>